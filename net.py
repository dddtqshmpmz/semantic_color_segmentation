...
class ResiduePredictor(nn.Module):
    ...
    def forward(self, target_img, mono_color_layers_pack):
        ...
        # 各レイヤーの各RGBチャンネルごとに，residueの平均が0になるように正規化する 对每一层的每个RGB通道进行归一化，使得残差的平均值为0。
...
class MaskGeneratorSeg2Color(nn.Module):

    def __init__(self, num_primary_color):
        super(MaskGeneratorSeg2Color, self).__init__()
        in_dim = 3 + num_primary_color * 3 + 1 # ex. 21 ch (= 3 + 6 * 3)
        out_dim = num_primary_color # num_out_layers is the same as num_primary_color.
    ...
    def forward(self, target_img, primary_color_pack, seg_output):
        x = torch.cat((target_img, primary_color_pack), dim=1)
        x = torch.cat((x,seg_output),dim=1)
...
class ResiduePredictorSeg2Color(nn.Module):
    def __init__(self, num_primary_color):
        super(ResiduePredictorSeg2Color, self).__init__()

        in_dim = 3 + num_primary_color * 4 + 1 # target画像を入力に追加している．alpha_layersをch方向に結合している
        out_dim = num_primary_color * 3
    ...
    def forward(self, target_img, mono_color_layers_pack, seg_output):
        # caution: mono_color_layers_pack is different from primary_color_pack.
        x = torch.cat((target_img, mono_color_layers_pack), dim=1)
        x = torch.cat((x, seg_output),dim=1)
