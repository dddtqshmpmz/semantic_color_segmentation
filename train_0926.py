...
from mydataset import MyDataset,MyDatasetMulti
import pytorch_ssim
import cv2
import os
import sys
...
# 让颜色分割网络收敛的慢一些 0926是1e-3 之后改成这个想试一下
parser.add_argument('--color_lr',default=1e-4, type=float,
...
train_dataset = MyDatasetMulti(args.csv_path_ihc ,args.num_primary_color, mode='train')
...
val_dataset = MyDatasetMulti( args.csv_path_ihc, args.num_primary_color, mode='val')
...
optimizer = optim.Adam(params, lr=config['color_lr'], betas=(0.0, 0.99)) # 1e-3 -> 0.2


# 加载新参数
model = smp.UnetWithColor('efficientnet-b3', in_channels= 7 ,
...
def train(epoch,min_train_loss):
    ...
    for batch_idx, (target_img, primary_color_layers, mask) in enumerate(train_loader):
        ...
        processed_alpha_layers = torch.squeeze(processed_alpha_layers,dim=2)
        output = model(torch.cat((target_img,processed_alpha_layers[:,:4,:,:]),1),processed_alpha_layers)
...
def val(epoch,min_val_loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (target_img, primary_color_layers,mask) in enumerate(val_loader):
            target_img = target_img.to(device) # bn, 3ch, h, w
            primary_color_layers = primary_color_layers.to(device)
            mask = mask.to(device)
