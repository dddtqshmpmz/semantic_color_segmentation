...
from mydataset import MyDataset, MyDatasetIHC, MyDatasetMulti, MyDatasetIHC_365, MyDatasetIHC2
import pytorch_ssim
import cv2
import os
import sys
...
init_batch_size = 20
init_batch_size_after = 24 #27  24
...
target_img_dir = '../unet_amax2/unet/inputs/patient_datasetRandom/images_10pics_30k_patches'
target_palette_dir = '../unet_amax2/unet/inputs/patient_datasetRandom/palette_10pics_30k_patches'
target_img_ids = glob(os.path.join(target_img_dir,'*'+'.png'))
target_img_ids = [ img_id.split('/')[-1].split('.')[0] for img_id in target_img_ids]

test_dataset = MyDatasetIHC2(target_img_ids,target_img_dir,target_palette_dir)
test_loader = torch.utils.data.DataLoader(
    test_dataset,
    batch_size=args.after_batch_size,  # 1
    shuffle=False,
    num_workers=args.num_workers,  # 1
    drop_last=False  # 加的这句
)
...
model_2 = smp.UnetWithColor_2('efficientnet-b3', in_channels=3+7,
                          classes=1, encoder_weights="imagenet").cuda()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (target_img, primary_color_layers, mask) in enumerate(val_after_loader):
            ...
            if (batch_idx % 10 == 0):
                ...
                save_layer_number = 0
                save_image(primary_color_layers[save_layer_number, :, :, :, :],
                           'results/val/' + 'img-%02d_primary_color_layers.png' % batch_idx)
                save_image(reconst_img[save_layer_number, :, :, :].unsqueeze(0),
                           'results/val/' + '_img-%02d_reconst_img.png' % batch_idx)
                save_image(target_img[save_layer_number, :, :, :].unsqueeze(0),
                           'results/val/' + '_img-%02d_target_img.png' % batch_idx)
                ...
                cv2.imwrite(os.path.join('results', 'val', 'semantic-output-%02d.png' % (batch_idx)),
...
def test():
    # generate pseu_label for chp4 (images_30pics_6k_patches), save to masks_30pics_6k_pseudo
    mask_generator.eval()
    residue_predictor.eval()
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (target_img, primary_color_layers, img_id) in enumerate(test_loader):
            target_img = target_img.to(device)  # bn, 3ch, h, w
            primary_color_layers = primary_color_layers.to(device)
            ...
            output = model(torch.cat((target_img.detach(), processed_alpha_layers_.detach()[
                           :, :, :, :]), 1), processed_alpha_layers_.detach())
            # output = model(target_img.detach())s

            pseu_dir = '../unet_amax2/unet/inputs/patient_datasetRandom/masks_10pics_30k_pseudo'
            # output the pred mask pics
            seg_output = torch.sigmoid(output)
            zero = torch.zeros_like(seg_output)
            one = torch.ones_like(seg_output)
            ...
            for i in range(0, len(seg_output)):
                cv2.imwrite(os.path.join(pseu_dir, img_id[i]+'.png'),
                            (seg_output[i, 0] * 255).astype('uint8'))
...
def test_for_paper():
    ...
    with torch.no_grad():
        ...
        for batch_idx, (target_img, primary_color_layers, mask) in enumerate(val_after_loader):
            ...
            if (batch_idx % 10 == 0):
                save_dir = 'results/val/train_1118_2'
                ...
                cv2.imwrite(os.path.join(save_dir, 'semantic-output-%02d.png' % (batch_idx)),
                            (seg_output[0, 0] * 255).astype('uint8'))
                cv2.imwrite(os.path.join(save_dir,'semantic-mask-%02d.png')%(batch_idx),
                            (mask[0,0]*255).astype('uint8'))
...
class FeatureExtractor():
    ...
    def __call__(self, x):  # 在对x求导时，将梯度保存下来
        ...
        # seg_output = torch.sigmoid(x)
        # zero = torch.zeros_like(seg_output)
        # one = torch.ones_like(seg_output)
        # seg_output = torch.where(seg_output > 0.5, one, zero)
        # seg_output = seg_output.data.cpu().numpy()
        # cv2.imwrite(os.path.join('results','heatmap','train_1118_2', 'x.png'),
        #             (seg_output[0, 0] * 255).astype('uint8')) 
        return outputs, x
...
class ModelOutputs():
    ...
    def __init__(self, model_2, processed_alpha_layers_, feature_module, target_layers):
        self.model_2 = model_2
        self.processed_alpha_layers_ = processed_alpha_layers_
    ...
    def __call__(self, x):
        target_activations = []
        x=model_2(torch.cat((x, self.processed_alpha_layers_), 1), self.processed_alpha_layers_)
        target_activations, x = self.feature_extractor(x)
        return target_activations, x


def show_cam_on_image(img, mask, save_dir, batch_idx,i):
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
    heatmap = np.float32(heatmap) / 255
    img = img.cpu().data.numpy().transpose((1,2,0))
    cam = heatmap + np.float32(img)
    cam = cam / np.max(cam)
    cv2.imwrite(os.path.join(save_dir,'cam_%02d_%02d.png') % (batch_idx,i), np.uint8(255 * cam))

class GradCam:
    def __init__(self, model_2, processed_alpha_layers_, feature_module, target_layer_names, use_cuda):
        self.model_2 = model_2
        self.processed_alpha_layers_ = processed_alpha_layers_
        self.feature_module = feature_module
        # self.model.eval()
        self.cuda = use_cuda
        if self.cuda:
            self.model_2 = model_2.cuda()

        self.extractor = ModelOutputs(self.model_2, self.processed_alpha_layers_, self.feature_module, target_layer_names)

    def forward(self, input):
        return self.model_2(torch.cat((input, self.processed_alpha_layers_), 1), self.processed_alpha_layers_)
...
def draw_heatmap_for_paper():
    ...
    for batch_idx, (target_img, primary_color_layers, mask) in enumerate(val_after_loader):
        ...
        if (batch_idx % 10 == 0):  # batch_idx % 10 == 0
            save_dir = 'results/heatmap/train_1124_base'
            ...
            batch_size = 1 # target_img.shape[0]
            for i in range(batch_size):
                grad_cam = GradCam(model_2 = model_2, processed_alpha_layers_=processed_alpha_layers_[i].unsqueeze(0), feature_module=model_2.segmentation_head, \
                        target_layer_names=["0"], use_cuda=True)
                heatmask = grad_cam(target_img[i].unsqueeze(0))
                show_cam_on_image(target_img[i],heatmask, save_dir, batch_idx, i)
...
if __name__ == "__main__":
    ...
    path_model = 'models/multitask/20211120/.pth'
    # model.load_state_dict(torch.load(path_model))
    model_2.load_state_dict(torch.load(path_model))

    # model = model.module
    model_2 = model_2.module

    draw_heatmap_for_paper()
